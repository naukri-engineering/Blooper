...
/**
 * This class handle all New Blooper database CRUD operations.
 *
 * @author akash
 */
public class ExceptionLogger {
    ...
    public void logException(ExceptionModel exceptionModel) throws SQLException {
        ...
        try {
            DatabaseOpenHelper databaseOpenHelper = DatabaseOpenHelper.getDbInstance(Blooper.getContext());
            ...
            if (databaseOpenHelper != null) {
                ...
                if (count < DATABASE_LOG_LIMIT) {
                    ...
                    if (sqLiteDatabase != null && exceptionModel != null) {
                        ...
                        {
                            BlooperLogger.info(LogMessage.DATABASE_EXCEPTION_INSERTED);
                        }
                    } else        // Database insertion error.
                    {
                        //Blooper.sendException(ExceptionLogger.class.getName().toString(), LogMessage.ERROR_DATABASE_INSERTION);
                        BlooperLogger.error(LogMessage.ERROR_EXCEPTION + LogMessage.ERROR_DATABASE_INSERTION);
                    }
                } else {
                    BlooperLogger.error(LogMessage.ERROR_EXCEPTION + LogMessage.ERROR_DATABASE_LIMIT_ERROR);
                }
            } else        // Database writable permission error.
            {
                //Blooper.sendException(ExceptionLogger.class.getName().toString(), LogMessage.ERROR_DATABASE_WRITABLE_PERMISSION);
                BlooperLogger.error(LogMessage.ERROR_EXCEPTION + LogMessage.ERROR_DATABASE_WRITABLE_PERMISSION);
            }
        } finally {
            if (cursor != null)
                cursor.close();
            ...
            if (databaseOpenHelper != null) {
                ...
                if (sqLiteDatabase != null) {
                    ...
                    if (cursor != null && cursor.moveToFirst()) {
                        ...
                        BlooperLogger.info(LogMessage.DATABSE_EXCEPTION_FETCH + cursorCount);
                    }
                } else        // Database fetch error.
                {
                    //Blooper.sendException(ExceptionLogger.class.getName().toString(), LogMessage.ERROR_DATABASE_FETCH);
                    BlooperLogger.error(LogMessage.ERROR_EXCEPTION + LogMessage.ERROR_DATABASE_FETCH);
                }
            } else        // Database readable permission error.
            {
                //Blooper.sendException(ExceptionLogger.class.getName().toString(), LogMessage.ERROR_DATABASE_READABLE_PERMISSION);
                BlooperLogger.error(LogMessage.ERROR_EXCEPTION + LogMessage.ERROR_DATABASE_READABLE_PERMISSION);
    ...
    public void clearData() {
        ...
        if (databaseOpenHelper != null) {
            ...
            if (sqLiteDatabase != null) {
                ...
                //Blooper.sendException(ExceptionLogger.class.getName().toString(), LogMessage.ERROR_DATABASE_INSERTION);
                BlooperLogger.error(LogMessage.ERROR_EXCEPTION + LogMessage.ERROR_DATABASE_DELETION);
            }
        } else        // Database writable permission error.
        {
            //Blooper.sendException(ExceptionLogger.class.getName().toString(), LogMessage.ERROR_DATABASE_WRITABLE_PERMISSION);
