package com.naukri.blooper.network;
...
public class WebServiceManager {
    ...
    public int sendException(NetworkRequest networkRequest) {
        ...
        try {
            BlooperLogger.info(LogMessage.HIT_WEB_SERVICE);
            String request = getRequestString(networkRequest);
            BlooperLogger.info(LogMessage.NWK_REQUEST + request);
            responseCode = sendRequest("", request, NetworkConstant.HTTP_METHOD_POST);
        } catch (Exception exception) {
            BlooperLogger.info(LogMessage.ERROR_EXCEPTION, exception);
        }
        return responseCode;
    }
    ...
    private int sendRequest(String serverURL, String jsonRequest, int httpMethod) {
        ...
        if (httpURLConnection != null) {
            try {
                ...
                if (Util.isSuccessResponse(responseCode)) //SUCCESS
                {
                    ...
                    //Close
                    inputStream.close();
                    BlooperLogger.info(LogMessage.SEND_SUCCESS + responseCode);
                } else //ERROR
                {
                    BlooperLogger.error(LogMessage.INCORRECT_RESPONSE_CODE + responseCode);
                    /*//Get Error Stream
                ...
                //Blooper.sendException(WebServiceManager.class.getName().toString(), ex);
                BlooperLogger.error(LogMessage.ERROR_SOCKET_TIMEOUT, ex);
            } catch (Exception ex) {
                //Blooper.sendException(WebServiceManager.class.getName().toString(), ex);
        ...
        BlooperLogger.info(LogMessage.NWK_RESPONSE + jsonResponse);
    ...
    private HttpURLConnection getHttpClient(String serverURL, int httpMethod) {
        ...
        try {
            ...
            //Blooper.sendException(WebServiceManager.class.getName().toString(), exception);
            BlooperLogger.error(LogMessage.ERROR_CONNECTION_TIME_OUT, exception);
        } catch (Exception exception) {
            //Blooper.sendException(WebServiceManager.class.getName().toString(), exception);
            BlooperLogger.info(LogMessage.ERROR_EXCEPTION, exception);
        }
...
//    }
   ...
   /* */
    ...
    private void prepareHeader(HttpURLConnection httpURLConnection) {
        BlooperConfiguration blooperConfiguration = Blooper.getBlooperConfiguration();

        Map headers = blooperConfiguration.httpHeaders;
